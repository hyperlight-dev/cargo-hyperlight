...
/// Data is saved to `./perf.data` by default (override with `-o`).
///
...
pub struct RecordArgs {
    ...
    /// Output perf.data path (default: ./perf.data.guest, or ./perf.data.kvm with --host).
    #[arg(short, long)]
...
pub fn run(args: RecordArgs) -> Result<()> {
    ...
    let default_output = if args.host {
        "perf.data.kvm"
    } else {
        "perf.data.guest"
    };
    ...
    if let Some(freq) = args.freq {
        eprintln!("Recording {mode_label} cycles @ {freq} Hz -> {output_display}");
    } else {
        eprintln!("Recording {mode_label} cycles -> {output_display}");
    }
    eprintln!(
        "Workload: {}",
        args.workload
    ...
    record_perf(&args, kallsyms_file.path())?;

    eprintln!();
    eprintln!("Data saved to {output_display}");
    ...
    if let Some(output) = &args.output {
        report_cmd.push_str(&format!(" -i {}", output.display()));
    }
    if args.host {
...
fn record_perf(args: &RecordArgs, kallsyms: &std::path::Path) -> Result<()> {
