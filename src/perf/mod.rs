...
fn generate_kallsyms(guest_binary: &Path, base_address: u64) -> Result<String> {
    ...
    // Sort by address, then by size descending, then by name for a
    // deterministic total order (stable across runs/toolchains).
    syms.sort_by(|a, b| a.0.cmp(&b.0).then(b.1.cmp(&a.1)).then(a.2.cmp(&b.2)));
    ...
    if let Some(addr) = text_addr {
        ...
        // Deterministic sort: markers (_text, _stext, __gap__) before real
        // symbols at the same address, so symbols__fixup_end() doesn't
        // shrink the first real function to zero length.
        final_syms.sort_by(|(addr_a, name_a), (addr_b, name_b)| {
            addr_a.cmp(addr_b).then_with(|| {
                fn marker_rank(name: &str) -> u8 {
                    match name {
                        "_text" | "_stext" | "__gap__" => 0,
                        _ => 1,
                    }
                }
                marker_rank(name_a)
                    .cmp(&marker_rank(name_b))
    ...
    for (addr, name) in &final_syms {
        // Write mangled names: the kallsyms format is whitespace-delimited
        // (address type name), so demangled names containing spaces (e.g.
        // generics like `<Foo as Bar>::method`) would be truncated by perf's
        // parser.  Demangling happens at report time in report.rs instead.
        writeln!(output, "{addr:016x} T {name}").unwrap();
    }
